...
export class CdkAppStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const securityGroup = this.createSecurityGroup('public-security-group', vpc);
    ...
    this.addIngressRule(securityGroup, 443, 'allow HTTPS traffic from anywhere');

    const s3Role = this.createRole('s3-readonly-role', 'AmazonS3ReadOnlyAccess');
    const linuxAmi = new ec2.AmazonLinuxImage({
      generation: ec2.AmazonLinuxGeneration.AMAZON_LINUX_2,
    });

    const publicEc2 = this.createT2MicroEc2('cdk-public-ec2', vpc, ec2.SubnetType.PUBLIC, 
      s3Role, securityGroup, linuxAmi, 'rgederin-lohika-2021-us-west-2');

    const userDataScript = readFileSync('./scripts/public_user_data.sh', 'utf8');
    publicEc2.addUserData(userDataScript)
  }

  createVpc: (vpcName: string, vpcCidr: string, subnetName: string) => ec2.Vpc =
    (vpcName: string, vpcCidr: string, subnetName: string) => {
  ...
  addIngressRule: (securityGroup: ec2.SecurityGroup, port: number, description: string) => void = 
    (securityGroup: ec2.SecurityGroup, port: number, description: string) => {
  ...
  createRole: (roleName: string, policyName: string) => iam.Role = 
    (roleName: string, policyName: string) => {
  ...
  createT2MicroEc2: (name: string, vpc: ec2.Vpc, subnetType: ec2.SubnetType, role: iam.Role,
    securityGroup: ec2.SecurityGroup, ami: ec2.AmazonLinuxImage, keyName: string) => ec2.Instance = 
      (name: string, vpc: ec2.Vpc, subnetType: ec2.SubnetType, role: iam.Role, 
        securityGroup: ec2.SecurityGroup, ami: ec2.AmazonLinuxImage, keyName: string) => {
